...
function Header({ user, onLogout, clerkUserButton, onLogoClick }) {
  ...
  return (
    <header className="bg-white dark:bg-gray-900 backdrop-blur-xl shadow-premium border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 transition-colors duration-300 overflow-x-hidden">
      <div className="container mx-auto px-2 sm:px-4 py-2 max-w-[100%] sm:max-w-[95%] 2xl:max-w-[1800px]">
        <div className="flex items-center justify-between">
          {/* Logo & Brand */}
          <div 
            className={`flex items-center space-x-2 sm:space-x-4 ${onLogoClick && user ? 'cursor-pointer' : ''} flex-shrink-0 min-w-0`}
            onClick={handleLogoClick}
          >
            <div className="w-12 h-12 sm:w-20 sm:h-20 overflow-hidden flex items-center justify-center flex-shrink-0">
              <img 
            ...
            <div className="flex flex-col min-w-0">
              <h1 className="text-lg sm:text-2xl font-bold tracking-tight truncate" style={{
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium tracking-wide mt-0.5 hidden sm:block">
                {t('appTagline')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-1 sm:space-x-3 flex-shrink-0">
            ...
            >
              ...
              <span className="text-xs sm:text-sm font-semibold text-purple-700 dark:text-purple-300 uppercase hidden sm:inline">{language}</span>
            </button>
