...
function Transactions({ initialFilters = {}, onFiltersCleared }) {
  ...
  // Rollover state
  const [rollingOverId, setRollingOverId] = useState(null);
  ...
  // Rollover handler - Shift income transaction to next month
  const handleRollover = async (transactionId) => {
    setRollingOverId(transactionId);
    setError('');

    try {
      const result = await rolloverTransaction(transactionId);
      ...
      console.error('Error rolling over transaction:', err);
      setError('Error al mover ingreso: ' + (err.response?.data?.error || err.message));
    } finally {
                  ...
                  <div className="flex items-center gap-1">
                    {transaction.type === 'income' && (
                      <button
                        ...
                        className="p-1.5 text-amber-600 hover:text-amber-700 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors duration-200 flex-shrink-0"
                        title="Mover al mes siguiente"
                        aria-label="Rollover to next month"
                      >
                        {rollingOverId === transaction.id ? (
                          <Loader className="w-4 h-4 animate-spin" />
                        ) : (
          ...
          <table className="table-premium w-full" style={{ minWidth: '640px', tableLayout: 'auto' }}>
            <thead>
              <tr>
                ...
                <th className="px-2 py-3 w-24 text-center text-base font-semibold text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredTransactions.length === 0 ? (
