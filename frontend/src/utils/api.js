...
export const updateCategoryBudget = async (categoryId, budgetAmount, categoryName = null, isAnnual = false) => {
...
// Rollover endpoint - Shift individual income transaction to next month
export const rolloverTransaction = async (transactionId) => {
  const response = await api.post(`/fix-nomina/rollover/${transactionId}`);
  return response.data;
};
